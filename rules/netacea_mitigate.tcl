...
when HTTP_RESPONSE {
  ...
  # Guard cross-event variables (HTTP::method/uri not available in HTTP_RESPONSE)
  if {![info exists method]} { set method "" }
  if {![info exists uri]} { set uri "" }
  if {![info exists useragent]} { set useragent "" }
  if {![info exists referer]} { set referer "" }
